<template>
  <div class="container">
    ...
    <div>
      ...
      <button v-for="method in methodNames" :key="method" @click="methodInvoke(method)">{{ method }}</button>
      <p>Function Result : {{ message }}</p>
    </div>
    <viewer class="viewer" :value="viewerText"/>
  </div>
</template>
...
export default {
  ...
  methods: Object.assign(eventListenr, {
    ...
    changePreviewStyle() {
      this.editorPreviewStyle =
        this.editorPreviewStyle === 'tab' ? 'vertical' : 'tab'
    }
  })
}
</script>
