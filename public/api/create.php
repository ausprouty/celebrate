...
function createContent($p){
	...
	if (!$text){
		...
		$sql = "INSERT into content (version,edit_date,edit_uid,language_iso,
			...
			'$country_code','$folder_name','$filetype','$title','$filename','$text')";
...
function createContentFolder($p){
	...
	$dir = ROOT_EDIT. '/content/'. $p['country_code']. '/'. $p['language_iso'] . '/'. $p['$folder_name'];
	$out['debug'] .= 'dir: ' . $dir ."\n";
	if (!file_exists($dir)){
		dirMake ($dir);
	}
...
function createDirectoryLanguages($p){
	...
	$dir_country = ROOT_EDIT.'/content/'. $p['country_code'] .'/';
...
function createDir($p){
	...
	switch ($scope){
		case 'country':
			$dir = ROOT_EDIT.'/content/'. $p['country_code'];
			break;
		case 'language':
		$dir = ROOT_EDIT.'/content/'. $p['country_code'] . '/'. $p['language_iso'];
			break;
		case 'folder':
			break;
	}
...
function createSeriesIndex($p){
	...
	$content = '[]';
	$file_index = ROOT_EDIT. 'content/' . $p['country_code'] . '/'. $p['language_iso'] . '/'. $p['folder_name'] .'/index.html';
...
function createStyle($p){
	...
	if ($valid){
		$dir = ROOT_EDIT. 'content/' . $p['country_code'] . '/styles/';
...
function createTemplate ($p){
	...
	if ($valid){
		if (isset($p['$folder_name'])){
			$dir = ROOT_EDIT. 'content/' . $p['country_code'] . '/'.  $p['language_iso'] . '/templates/'. $p['$folder_name'] ;
			if (!file_exists($dir)){
				dirMake ($dir);
			}
			$fname = $dir . '/'. $_FILES["file"]["name"];
